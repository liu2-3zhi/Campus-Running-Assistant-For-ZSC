...
- 前端只返回当前实际需要的地图供应商密钥；`map_providers` 只能包含脱敏元数据，密钥密文放在 `map_provider_key_bundle` 中，不能一次性下发所有供应商密钥。
- RSA 密钥对只保存在服务进程内存中，服务每次启动都必须生成新的密钥对；私钥只通过 `/api/map_key_runtime.js?v=<runtime_version>` 对当前会话下发，且响应必须禁止缓存。
...
- 不要把 `map_key_runtime.js` 作为普通 `/scripts/<filename>` 静态资源提供；解密脚本必须走 API，以避免 CDN 或静态缓存返回错误上下文。
